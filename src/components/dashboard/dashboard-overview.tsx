...
export function DashboardOverviewClient({
  ...
  return (
    <div className="flex flex-1 flex-col gap-6 p-4 sm:p-6 lg:p-8 max-w-[1440px] mx-auto w-full">
          ...
          <div className="flex flex-col gap-2.5">
            <div className="flex flex-wrap items-center gap-2.5">
              ...
              <Badge
                variant="outline"
                className={cn(
                  "px-2.5 py-0.5 text-xs font-semibold gap-1.5 shadow-2xs",
                  user.role === "OWNER" && "border-amber-500/30 bg-amber-500/10 text-amber-600 dark:text-amber-400",
                  user.role === "MANAGER" && "border-indigo-500/30 bg-indigo-500/10 text-indigo-600 dark:text-indigo-400",
                  user.role === "CASHIER" && "border-emerald-500/30 bg-emerald-500/10 text-emerald-600 dark:text-emerald-400"
                )}
              >
                {user.role === "OWNER" && <Crown className="size-3.5" />}
                {user.role === "MANAGER" && <ShieldCheck className="size-3.5" />}
                {user.role === "CASHIER" && <Calculator className="size-3.5" />}
            ...
            <div className="flex items-center gap-2 mt-1 text-xs font-medium">
              {activeShift ? (
                <span className="inline-flex items-center gap-2 rounded-full border border-emerald-500/30 bg-emerald-500/10 px-3 py-1 text-emerald-600 dark:text-emerald-400 shadow-2xs">
                  ...
                  <span className="opacity-70">({activeShift.cashierName})</span>
                </span>
              ) : (
      ...
      {/* ────────────────── 3. Main Dashboard Body (2 Cols) ────────────────── */}
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        {/* Left Column: Recent Orders Feed (8 Cols) */}
          ...
          <Card className="border-border/70 shadow-xs p-4">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-5 gap-2 text-center">
              <div className="rounded-lg border border-blue-500/20 bg-blue-500/5 p-2.5">
                <span className="text-[11px] text-blue-600 dark:text-blue-400 font-semibold block">جديد</span>
                <span className="text-xl font-bold font-mono tabular-nums text-foreground">{statusCounts.NEW}</span>
              </div>
              <div className="rounded-lg border border-amber-500/20 bg-amber-500/5 p-2.5">
                <span className="text-[11px] text-amber-600 dark:text-amber-400 font-semibold block">قيد التحضير</span>
                <span className="text-xl font-bold font-mono tabular-nums text-foreground">{statusCounts.PREPARING}</span>
              </div>
              <div className="rounded-lg border border-emerald-500/20 bg-emerald-500/5 p-2.5">
                <span className="text-[11px] text-emerald-600 dark:text-emerald-400 font-semibold block">جاهز</span>
              ...
              <div className="rounded-lg border border-purple-500/20 bg-purple-500/5 p-2.5">
                <span className="text-[11px] text-purple-600 dark:text-purple-400 font-semibold block">في الطريق</span>
              ...
              <div className="rounded-lg border border-green-600/20 bg-green-600/5 p-2.5 col-span-2 sm:col-span-1">
                <span className="text-[11px] text-green-700 dark:text-green-300 font-semibold block">تم التسليم</span>
        ...
        {/* Right Column: Operations Hub, Brands, & Platforms (4 Cols) */}
