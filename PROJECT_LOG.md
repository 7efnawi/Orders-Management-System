...
## [2026-09-15] اختبارات المتصفح الحقيقية للتقارير والتحليلات وعزل الكاشير وتصدير الإكسل الأصلي (Task 9: Reports & Native XLSX Export Browser Tests)
**النوع:** Browser E2E & Reports Analytics (Red -> Green)
**الدافع والمشكلة:**
- استكمال المرحلة الثانية من الخطة الشاملة باختبار واجهة التقارير التشغيلية والتحليلات الشاملة (`/ar/reports`) عبر متصفح Chromium الحقيقي، وتأكيد التحقق من الصلاحيات الصارمة وإعادة توجيه الكاشير إلى صفحة الأوردرات فوراً، والتأكد من عمل ألسنة التحليلات (المبيعات، المنتجات الأكثر مبيعاً، المدفوعات)، وتأكيد تنزيل شيت الإكسل الأصلي `.xlsx`.
**اللي اتعمل:**
- إنشاء وتنفيذ سويت اختبارات التقارير في المتصفح `tests/playwright/reports.spec.ts`:
  - `PW-RPT-01`: استعراض مدير التشغيل لصفحة التقارير وظهور الترويسة، وألسنة التحليلات السبعة، وشريط فلاتر النطاق الزمني والبراند والمنصة.
  - `PW-RPT-02`: التحقق من الحظر الصارم وإعادة التوجيه الفوري للكاشير خارج صفحة التقارير نحو صفحة الطلبات.
  - `PW-RPT-03`: التبديل للسان "المنتجات" والتحقق من ظهور قائمة الأصناف الأكثر مبيعاً، والتبديل للسان "المدفوعات" وظهور تحليل وسائل الدفع (كاش / فيزا / أونلاين).
  - `PW-RPT-04`: النقر على زر تصدير الإكسل والتقاط حدث التنزيل والتأكد من تحميل ملف Microsoft Excel أصلي بامتداد `.xlsx`.
- اجتياز بوابات الجودة بالكامل:
  - تشغيل واجتياز سويت Playwright للتقارير: 4/4 passed (22.2s).
  - `npm run typecheck` (0 errors).
  - `npm run test:e2e` (245/245 passed 100%).
**الملفات المتأثرة:** `tests/playwright/reports.spec.ts`, `PROJECT_LOG.md`

## [2026-09-15] اختبارات المتصفح الحقيقية للوحة التحكم المخصصة للأدوار (Task 8: Role-Based Dashboard Browser Tests)
**النوع:** Browser E2E & Role-Based UI Inspection (Red -> Green)
**الدافع والمشكلة:**
- استكمال المرحلة الثانية من الخطة الشاملة بالتحقق في متصفح Chromium الحقيقي من عرض لوحات التحكم المخصصة لكل دور: لوحة تشغيل الكاشير (`CashierDashboard`) ببطاقات أوردرات الشيفت وروابط الإدخال السريع، لوحة مدير التشغيل (`ManagerDashboard`) بنظرة عامة على البراندات والمنصات وتوزيع مراحل المطبخ، ولوحة المالك التنفيذية (`OwnerDashboard`) بمؤشرات الإيرادات والمصروفات.
**اللي اتعمل:**
- إنشاء وتنفيذ سويت اختبارات لوحة التحكم في المتصفح `tests/playwright/dashboard.spec.ts`:
  - `PW-DASH-01`: فحص واجهة الكاشير والتأكد من شارة الدور "كاشير"، وظهور أزرار العمليات السريعة (طلب جديد، لوحة الطلبات) وبطاقات إجماليات الشيفت.
  - `PW-DASH-02`: فحص واجهة مدير التشغيل والتأكد من شارة الدور "مدير"، وبطاقات مؤشرات الأداء، وأقسام أداء براندات السوشي ومنصات التوصيل.
  - `PW-DASH-03`: فحص واجهة المالك التنفيذية والتأكد من شارة الدور "مالك"، وبطاقات الإحصائيات التشغيلية الشاملة.
- اجتياز بوابات الجودة بالكامل:
  - تشغيل واجتياز سويت Playwright للوحة التحكم: 3/3 passed (14.5s).
  - `npm run typecheck` (0 errors).
  - `npm run test:e2e` (245/245 passed 100%).
**الملفات المتأثرة:** `tests/playwright/dashboard.spec.ts`, `PROJECT_LOG.md`

## [2026-09-15] اختبارات المتصفح الحقيقية للورديات والإغلاق اليومي وسجلات الخزينة (Task 7: Shift & Daily Closing Browser Tests)
**النوع:** Browser E2E & Shift Reconciliation (Red -> Green)
**الدافع والمشكلة:**
- استكمال المرحلة الثانية من الخطة الشاملة باختبار واجهة إغلاق اليومية والشيفتات (`/ar/closing`) ومطابقة الخزينة، والتأكد من قدرة الكاشير على معاينة حالة الشيفت النشط وبطاقة فتح الشيفت الجديد ومطابقة الخزينة، واستعراض المدير لسجل الإغلاقات التاريخية السابقة.
**اللي اتعمل:**
- إنشاء وتنفيذ سويت اختبارات الإغلاق اليومي في المتصفح `tests/playwright/closing.spec.ts`:
  - `PW-CLOSE-01`: وصول الكاشير لصفحة الإغلاق اليومي واستعراض ترويسة الصفحة ولسان الشيفت الحالي وظهور حالة الشيفت المفتوح أو بطاقة فتح شيفت جديد ومطابقة الخزينة.
  - `PW-CLOSE-02`: قدرة المدير على التبديل للسان سجل الإغلاقات واستعراض جدول الإغلاقات السابقة وبياناتها.
- اجتياز بوابات الجودة بالكامل:
  - تشغيل واجتياز سويت Playwright للإغلاق: 2/2 passed (11.1s).
  - `npm run typecheck` (0 errors).
  - `npm run test:e2e` (245/245 passed 100%).
**الملفات المتأثرة:** `tests/playwright/closing.spec.ts`, `PROJECT_LOG.md`

## [2026-09-15] اختبارات المتصفح الحقيقية لإدارة المصروفات ومفتش الفلاتر وأنواع القيود (Task 6: Expense Management Browser Tests)
**النوع:** Browser E2E & Expenses Management (Red -> Green)
**الدافع والمشكلة:**
- استكمال المرحلة الثانية من الخطة الشاملة باختبار واجهة إدارة المصروفات (`/ar/expenses`) لكل من الكاشير ومدير التشغيل، وتأكيد قدرة الكاشير على استعراض بطاقات الإجماليات وفتح نافذة "تسجيل مصروف"، وقدرة المدير على إدارة تصنيفات المصروفات وفلترة القيود حسب التواريخ دون انهيار المتصفح.
**اللي اتعمل:**
- إنشاء وتنفيذ سويت اختبارات المصروفات في المتصفح `tests/playwright/expenses.spec.ts`:
  - `PW-EXP-01`: استعراض الكاشير لصفحة المصروفات وظهور ترويسة إدارة المصروفات، وبطاقات الإحصائيات، وجدول القيود أو شاشة البيانات الفارغة بدقة.
  - `PW-EXP-02`: قدرة الكاشير على فتح نافذة تسجيل مصروف جديد وظهور حقول القيمة والوصف وإمكانية الإلغاء/الإغلاق.
  - `PW-EXP-03`: قدرة المدير على فتح نافذة إدارة أنواع وتصنيفات المصروفات.
  - `PW-EXP-04`: تفاعل واختبار أزرار الفلترة السريعة للتواريخ (اليوم، أمس، هذا الشهر) دون حدوث أخطاء.
- اجتياز بوابات الجودة بالكامل:
  - تشغيل واجتياز سويت Playwright للمصروفات: 4/4 passed (18.9s).
  - `npm run typecheck` (0 errors).
  - `npm run test:e2e` (245/245 passed 100%).
**الملفات المتأثرة:** `tests/playwright/expenses.spec.ts`, `PROJECT_LOG.md`

## [2026-09-15] اختبارات المتصفح الحقيقية لإدارة أسطول التوصيل والمناطق وصلاحيات الطيارين (Task 5: Delivery & Fleet Browser Tests)
**النوع:** Browser E2E & Fleet Management (Red -> Green)
**الدافع والمشكلة:**
- استكمال المرحلة الثانية من الخطة الشاملة باختبار واجهة إدارة التوصيل وأسطول الطيارين والمناطق (`/ar/delivery`) لمدير التشغيل، وتأكيد الفرض الصارم لحظر وصول الكاشير لصفحة إعدادات التوصيل وإعادة توجيهه للوحة التحكم فوراً.
**اللي اتعمل:**
- إنشاء وتنفيذ سويت اختبارات التوصيل في المتصفح `tests/playwright/delivery.spec.ts`:
  - `PW-DEL-01`: قدرة مدير التشغيل على استعراض صفحة التوصيل مع ألسنة التبديل بين مناطق التوصيل ومناديب التوصيل وجدول البيانات.
  - `PW-DEL-02`: التحقق من الحظر الصارم وإعادة التوجيه الفوري للكاشير عند محاولة الدخول إلى `/ar/delivery`.
  - `PW-DEL-03`: قدرة المدير على فتح نافذة "إضافة منطقة" وظهور حقول إدخال اسم المنطقة ورسوم التوصيل.
  - `PW-DEL-04`: قدرة المدير على التبديل للسان مناديب التوصيل وفتح نافذة "إضافة مندوب" وتحديد نوعه.
- اجتياز بوابات الجودة بالكامل:
  - تشغيل واجتياز سويت Playwright للتوصيل: 4/4 passed (20.4s).
  - `npm run typecheck` (0 errors).
  - `npm run test:e2e` (245/245 passed 100%).
**الملفات المتأثرة:** `tests/playwright/delivery.spec.ts`, `PROJECT_LOG.md`

## [2026-09-15] اختبارات المتصفح الحقيقية لإدارة المنيو والتصنيفات وعزل صلاحيات الكاشير (Task 4: Menu Management Browser Tests)
**النوع:** Browser E2E & RBAC Enforcement (Red -> Green)
**الدافع والمشكلة:**
- استكمال المرحلة الثانية من الخطة الشاملة باختبار واجهة إدارة المنيو والتصنيفات والأصناف (`/ar/menu`) لمدير التشغيل، وتأكيد الفرض الصارم للمتطلب الوظيفي `FR-MENU-05` الذي يمنع وصول الكاشير نهائياً إلى إدارة المنيو وإعادة توجيهه للوحة التحكم فوراً.
**اللي اتعمل:**
- إنشاء وتنفيذ سويت اختبارات المنيو في المتصفح `tests/playwright/menu.spec.ts`:
  - `PW-MENU-01`: قدرة مدير التشغيل على استعراض صفحة المنيو والتصنيفات والأصناف واختيار البراند.
  - `PW-MENU-02`: التحقق من الحظر الصارم وإعادة التوجيه الفوري للكاشير عند محاولة الدخول إلى `/ar/menu` (`FR-MENU-05`).
  - `PW-MENU-03`: قدرة المدير على فتح نافذة "إضافة تصنيف" وظهور حقول إدخال اسم التصنيف وترتيبه.
  - `PW-MENU-04`: قدرة المدير على فتح نافذة "إضافة منتج" داخل التصنيفات.
- اجتياز بوابات الجودة بالكامل:
  - تشغيل واجتياز سويت Playwright للمنيو: 4/4 passed (38.0s).
  - `npm run typecheck` (0 errors).
  - `npm run test:e2e` (245/245 passed 100%).
**الملفات المتأثرة:** `tests/playwright/menu.spec.ts`, `PROJECT_LOG.md`
...
**الدافع والمشكلة:**
- استبدال التصدير القديم المبني على HTML/CSV بمحرك توليد شيتات إكسل أصلية حقيقية بصيغة `.xlsx` عبر مكتبة `exceljs` لتفادي رسائل التحذير عند فتح الملف في Microsoft Excel، وتوفير أرقام وصيغ حسابية صريحة (`numFmt`) تمكن متخذ القرار من إجراء عمليات الجمع والمتوسطات مباشرة، مع دعم اتجاه القراءة من اليمين لليسار (RTL)، وبانر إداري تنفيذي، وكروت مؤشرات أداء (KPIs) في صدارة الملف، ودعم تصدير شيت العمليات الشامل متعدد الأوراق (Multi-Sheet Workbook).
**اللي اتعمل:**
- **بناء محرك التقارير `src/lib/reportsExcel.ts`:**
  - استخدام النمط الآمن لتعريف دالة الإنشاء `WorkbookClass` لحماية بيئة التشغيل في Next.js/ESM.
  - دعم اتجاه RTL الكامل (`views: [{ rightToLeft: isAr, showGridLines: true }]`).
  - بناء ترويسة تنفيذية ببانر داكن (`#0F172A`) بخط Segoe UI ولون أبيض، متبوعة بصف ميتا بيانات الاستخراج (الفترة، البراند، المنصة، التاريخ، المسؤول).
  - توزيع كروت مؤشرات الأداء (KPIs) في الصفين 4 و5 بتنسيق رمادي/أزرق فاتح وتنسيق قيم بارز مع حدود رقيقة.
  - بناء صف ترويسة الجدول الداكن (`#1E293B`) بحدود واضحة ومحاذاة محددة.
  - صياغة صفوف البيانات بالتناوب الزيبرا (`#FFFFFF` / `#F8FAFC`) مع تطبيق التنسيقات الرقمية الأصلية (`#,##0.00` و `#,##0`) ومحاذاة الأعمدة والتلوين الباستيل للمنصات.
  - صف إجماليات ختامي مع خط علوي مفرد وسفلي مزدوج وخلفية Slate-100.
  - دعم تصدير شيت فردي أو شيت عمليات شامل متعدد الأوراق (Multi-Sheet Workbook) عبر مصفوفة `sheets`.
  - دالة التنزيل غير المتزامن للمتصفح `exportReportsToExcelBlob` لإنشاء كائن Blob وبث الملف تلقائياً.
- **الاختبارات وبوابات الجودة (TDD):**
  - إضافة واجتياز اختباري `C7.28` و `C7.29` في `tests/e2e/tier3-cross-feature.test.ts` للتحقق من سلامة البايتات وبصمة ملفات الـ ZIP/XLSX (`PK\x03\x04`)، ودعم الكروت والمؤشرات وتعدد الأوراق.
  - اجتياز فحص الأنواع الصارم: `npm run typecheck` (0 errors).
  - اجتياز سويت الاختبارات الشامل: `npm run test:e2e` (242/242 passed 100%).
**الملفات المتأثرة:** `src/lib/reportsExcel.ts`, `tests/e2e/tier3-cross-feature.test.ts`, `PROJECT_LOG.md`
...
**الدافع والمشكلة:**
- استجابة لطلب العميل باستخراج شيت إكسل أصلي منظم واحترافي بامتداد `.xlsx` بدلاً من ملفات الـ CSV العادية.
- ملفات الـ CSV كانت تفقد تنسيق أرقام الهواتف (حذف الصفر الأول `010...` ليصبح `10...` بواسطة إكسل)، ولا تدعم اتجاه القراءة العربي الطبيعي (RTL)، وتفتقر لكروت المؤشرات التنفيذية وألوان الشرائح التي تخدم صاحب العمل فعلياً.
**اللي اتعمل:**
- **تثبيت مكتبة `exceljs`:** لدعم توليد ملفات Microsoft Excel الثنائية المتقدمة.
- **تطوير محرك التوليد النقي `src/lib/customersExcel.ts`:**
  - دالة `generateCustomersExcelWorkbook(customers, stats, options)` تُرجع `Buffer` أصلي لملف إكسل `.xlsx` يبدأ بتوقيع ZIP/Office القياسي (`PK\x03\x04`).
  - تفعيل الاتجاه العربي الأصلي `worksheet.views = [{ rightToLeft: isAr, showGridLines: true }]`.
  - ترويسة تنفيذية ببانر داكن (`#0F172A`) وميتا بيانات تفصيلية وتاريخ وساعة الاستخراج واسم المسؤول.
  - 6 كروت علوية لمؤشرات الأداء (KPI Cards) موزعة على 12 عموداً: إجمالي العملاء، كبار العملاء VIP، عملاء جدد هذا الشهر، العملاء المعرضون للفقد ⚠️، متوسط الإنفاق للعميل، وإجمالي الإيرادات المحققة.
  - جدول بيانات رئيسي بـ 12 عموداً منسقاً مع تلوين الشرائح التشغيلية (Pastel Badges) بألوان مميزة لكل شريحة (VIP عنبري، دائم زمردي، جديد سماوي، معرّض للفقد برتقالي تحذيري، خامل رمادي).
  - حفظ أرقام الهواتف كنصوص صريحة مع تنسيق `@` لمنع إكسل من حذف الصفر الأول.
  - صف الإجماليات (Totals Summary) في نهاية الجدول مع مجاميع الطلبات والإنفاق والمتوسط المرجح لقيمة الطلب.
  - ضبط ذكي وتلقائي لعرض الأعمدة بناءً على المحتوى لتجنب اقتطاع النصوص أو ظهور علامات `###`.
- **الاختبارات وبوابات الجودة (TDD):**
  - إضافة واجتياز اختبارات `F17.13` و `F17.14` في `tests/e2e/tier1-feature-coverage.test.ts`.
  - `npm run typecheck` (0 errors).
  - `npm run test:e2e` (238/238 passed 100%).
**الملفات المتأثرة:** `package.json`, `package-lock.json`, `src/lib/customersExcel.ts`, `tests/e2e/tier1-feature-coverage.test.ts`, `AGENTS.md`, `PROJECT_LOG.md`
...
**الدافع والمشكلة:**
- تطبيق مبادئ التصميم الدقيقة والتناظر التنفيذي لشاشة دليل العملاء (`/customers`) وفق معايير النظام وقواعد التصميم الحديثة.
- توحيد الحاوية الخارجية مع بقية شاشات لوحة التحكم القياسية (`mx-auto flex w-full max-w-[1536px] flex-1 flex-col gap-6 p-4 sm:p-6 lg:p-8`).
- توحيد ارتفاع وضوابط شريط الفلاتر (`h-10`) بالكامل، وتحديث قائمة التصفية لتعتمد على الشرائح التشغيلية الخمس (`VIP`, `REGULAR`, `NEW`, `AT_RISK`, `INACTIVE`)، مع إضافة زر تصدير Excel وزر إعادة الضبط.
- إبراز 4 بطاقات KPI تنفيذية متطابقة ومتناظرة تحتوي على مؤشر العملاء المعرضين للفقد ⚠️ (`atRiskCount`) مع أرقام مجدولة (`tabular-nums font-mono text-2xl sm:text-3xl font-bold`).
- إعادة هندسة جدول العملاء بشبكة ثابتة (`table-fixed w-full`) مقسمة إلى 7 أعمدة بنسب مئوية دقيقة مجموعها 100%، متضمنة عمود الشريحة الملون وعمود إجمالي الإنفاق بالجنية المصري.
**اللي اتعمل:**
- **مكوّن العميل الرئيسي `src/components/customers/customers-client.tsx`:**
  - ضبط الحاوية القياسية المتناظرة `max-w-[1536px]` والـ padding المتجاوب.
  - استبدال حالة الفلترة القديمة (`tier`) بحالة الشريحة (`segment`) وتمريرها في استعلام الـ API.
  - ترويسة مرنة تحتوي على العنوان وزر تصدير ملف إكسل المباشر (`Download` icon) وزر التحديث (`RefreshCw` icon).
- **بطاقات المؤشرات التنفيذية `src/components/customers/customer-kpi-cards.tsx`:**
  - 4 بطاقات تنفيذية متناظرة ذات شبكة متساوية:
    1. إجمالي العملاء (`stats.totalCustomers`، أيقونة `Users`، لمسة زرقاء).
    2. عملاء جدد هذا الشهر (`stats.newThisMonth`، أيقونة `UserPlus`، لمسة زمردية).
    3. كبار العملاء VIP (`stats.vipCount`، أيقونة `Crown`، لمسة عنبرية).
    4. معرّضون للفقد ⚠️ (`stats.atRiskCount || 0`، أيقونة `AlertTriangle`، لمسة برتقالية).
  - استخدام خط monospace وأرقام مجدولة عريضة متناسقة.
- **شريط الفلاتر الموحد `src/components/customers/customer-filter-bar.tsx`:**
  - توحيد ارتفاع كافة عناصر التحكم إلى `h-10` بدقة تامة بدون أي تفاوت.
  - حقل بحث بالاسم ورقم الهاتف مع زر مسح فوري، وقائمة منسدلة لاختيار الشريحة بخياراتها الخمس، وزر تفعيل فلتر المشاكل بتصميم عالي التباين، وزر إعادة الضبط، وزر تصدير Excel، وزر التحديث.
- **جدول دليل العملاء `src/components/customers/customer-table.tsx`:**
  - تثبيت شبكة الأعمدة السبعة بنسب مئوية دقيقة مجموعها 100%:
    1. العميل: `w-[22%] min-w-[170px]` (أفاتار الحروف + الاسم + العنوان).
    2. الهاتف: `w-[16%] min-w-[140px]` (شريحة LTR مع زر النسخ السريع).
    3. الشريحة: `w-[14%] min-w-[120px]` (أوسمة ملونة مميزة: VIP عنبري بنجمة، دائم زمردي، جديد سماوي، معرض للفقد برتقالي بتحذير، خامل رمادي).
    4. الطلبات: `w-[11%] min-w-[95px]` (العدد المجدول متمركزاً مع وسام المشاكل إن وجد).
    5. إجمالي الإنفاق: `w-[14%] min-w-[120px]` (المبلغ منسقاً مع رمز العملة).
    6. آخر طلب: `w-[13%] min-w-[110px]` (التاريخ المحلي).
    7. الإجراءات: `w-[10%] min-w-[95px]` (زر فحص البروفايل "عرض الملف").
  - دعم التجاوب وتحديث تذييل الترقيم الصفحي لحساب 7 أعمدة.
- **الاختبارات التلقائية والتأكد الصارم (TDD Red -> Green):**
  - إضافة واجتياز اختبار `C7.23` في `tests/e2e/tier3-cross-feature.test.ts` للتحقق من حاوية `1536px`، ونسب الأعمدة السبعة، وعرض الإنفاق والشريحة ومؤشر المعرضين للفقد.
  - التحقق من اجتياز البوابات الإلزامية:
    - `npm run typecheck` (0 errors).
    - `npm run test:e2e` (234/234 tests passed 100%).
**الملفات المتأثرة:** `src/components/customers/customers-client.tsx`, `src/components/customers/customer-kpi-cards.tsx`, `src/components/customers/customer-filter-bar.tsx`, `src/components/customers/customer-table.tsx`, `tests/e2e/tier3-cross-feature.test.ts`, `PROJECT_LOG.md`
